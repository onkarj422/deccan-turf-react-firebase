...
// Fetch all turfs
export const useFetchTurfs = () => useQuery({
    queryKey: ['turfs'],
    queryFn: getAllTurfs,
});

// Fetch a single turf by ID
export const useFetchTurfById = (turfId: string) => useQuery({
...
export const useUpdateTurf = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ turfId, updatedData }: { turfId: string; updatedData: Partial<Turf> }) => updateTurf(turfId, updatedData),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['turfs'] }); // Invalidate the turfs query to refetch data
        },
    });
