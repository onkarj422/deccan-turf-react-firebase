...
// Fetch all slots
export const useFetchSlots = () => useQuery({
    queryKey: ['slots'],
    queryFn: getAllSlots,
});

// Fetch a single slot by ID
export const useFetchSlotById = (slotId: string) => useQuery({
...
export const useUpdateSlot = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ slotId, updatedData }: { slotId: string; updatedData: Partial<Slot> }) => updateSlot(slotId, updatedData),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['slots'] }); // Invalidate the slots query to refetch data
        },
    });
